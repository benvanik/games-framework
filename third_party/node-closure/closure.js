...
// MODIFICATIONS BY benvanik@google.com:
// - various tweaks for correct paths
// - support searching anvil build-* paths
// - adapted for closure_js_library deps output
...
exports.Closure = function(opt_goog_global) {
  ...
  */
  function load_script(filename) {
    ...
    var searchPaths = ['build-out', 'build-gen'];
    for (var n = 0; n < searchPaths.length; n++) {
      var searchPath = searchPaths[n];
      searchFilename = path.join(searchPath, filename);
      try {
        fs.statSync(searchFilename);
      } catch (e) {
        continue;
      }
      var code = fs.readFileSync(searchFilename, 'utf8');
